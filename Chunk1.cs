...
public class Chunk {
	...
	string BuildChunkFileName(Vector3 v)
	{
		return Application.persistentDataPath + "/savedata/Chunk_" +
								(int)v.x + "_" +
								(int)v.y+"_"+
								(int)v.z+"_"+World.chunkSize+"_"+World.radius+".dat";
	}
	...
	bool Load()
	{
		...
		if(File.Exists(chunkFile))
		{
			...
			file.Close();
			Debug.Log("Loaded from " + chunkFile);
			return true;
		}
		//Debug.Log("couldn't load from " + chunkFile);
		return false;
	}
			...
			for(int y = 0; y < World.chunkSize; y++)
				for(int x = 0; x < World.chunkSize; x++)
				{
					...
					if(dataFromFile)
					{
						...
						continue; // basically break I guess, skips the rest and goes to the next part of the loop.
					}
					...
					else if(worldY <= Utils.GenerateStoneHeight(worldX,worldZ)) // we start from the bottom since we are building from the bottom up, <=
					{	
						...
						else if(Utils.fBM3D(worldX,worldY,worldZ,0.03f,3) < 0.41 && worldY < 20)
